...
import { list_All_Questions, create_a_New_Question } from './api.js';
...
function requireAuth() {
  if (!isConfigured()) {
    ...
    console.log(chalk.cyan('  clickupcom config set --api-key YOUR_API_KEY'));
    process.exit(1);
  }
}
...
program
  .name('clickupcom')
  .description(chalk.bold('clickup20 CLI') + ' - Production-ready CLI')
  .version('1.0.0');
...
program
  ...
  .action(async (options) => {
    ...
    try {
      const data = await withSpinner('Making API call...', () => list_All_Questions());
